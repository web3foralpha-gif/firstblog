...
export default function CommentsPage() {
  ...
  async function action(id: string, act: string) {
    await fetch(`/api/houtai/comments/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: act }) })
  ...
  async function bulkAction(act: string) {
    ...
    await Promise.all(Array.from(selected).map(id =>
      fetch(`/api/houtai/comments/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: act }) })
    ))
