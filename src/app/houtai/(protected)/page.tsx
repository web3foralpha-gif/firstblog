...
import { Card, Badge } from '@/components/houtai/ui'
...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: '已发布文章', value: articleCount,           href: '/houtai/articles',  icon: '📄', color: 'text-blue-700',   bg: 'bg-blue-50',   border: 'border-blue-100' },
    { label: '待审评论',   value: pendingComments,        href: '/houtai/comments',  icon: '💬', color: 'text-amber-700',  bg: 'bg-amber-50',  border: 'border-amber-100' },
    { label: '待审留言',   value: pendingGuestbook,       href: '/houtai/guestbook', icon: '📮', color: 'text-violet-700', bg: 'bg-violet-50', border: 'border-violet-100' },
    { label: '累计打赏',   value: `¥${(revenue/100).toFixed(2)}`, href: '/houtai/payments', icon: '💳', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-100' },
    { label: '向日葵照顾', value: `${sunflowerState?.totalCount ?? 0}人`, href: '/houtai/sunflower', icon: '🌻', color: 'text-green-700', bg: 'bg-green-50', border: 'border-green-100' },
    { label: '今日访问',   value: pageViewsToday,         href: '/houtai/analytics', icon: '📊', color: 'text-slate-700',  bg: 'bg-slate-50',  border: 'border-slate-100' },
  ]
      ...
      <Card>
        ...
        <div className="divide-y divide-slate-50">
          {recentArticles.length === 0 ? (
            ...
            <Link key={a.id} href={`/houtai/articles/${a.id}/edit`}
        ...
        <div className="p-4 border-t border-slate-50">
          <Link href="/houtai/articles" className="text-xs text-slate-400 hover:text-slate-600 transition-colors">
            查看全部文章 →
          </Link>
        </div>
      </Card>
