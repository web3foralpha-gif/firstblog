...
export default function ArticleEngagementBar({
  ...
  async function toggleLike() {
    try {
      ...
      if (!res.ok || !data?.summary) {
        throw new Error(String(data?.error || copy.likeError))
      }
      setSummary(data.summary)
    } catch (error) {
      setFlash({ type: 'error', message: error instanceof Error ? error.message : copy.likeError })
    } finally {
      setBusy(null)
    }
  }
  ...
  async function copyShareLink() {
    try {
      ...
      setFlash({ type: 'success', message: copy.copyLinkSuccess })
    } catch {
      setFlash({ type: 'error', message: copy.copyLinkError })
    } finally {
      setBusy(null)
    }
  }
  ...
  async function openShareImagePreview() {
    try {
      ...
      if (!res.ok) {
        throw new Error(copy.posterGenerateError)
      }
      ...
      setFlash({ type: 'success', message: copy.posterGenerateSuccess })
    } catch (error) {
      setFlash({ type: 'error', message: error instanceof Error ? error.message : copy.posterGenerateError })
    } finally {
      setBusy(null)
    }
  }
  ...
  async function copyShareImage() {
    ...
    try {
      ...
      if (typeof ClipboardItem === 'undefined' || !navigator.clipboard?.write) {
        throw new Error(copy.copyPosterUnsupportedError)
      }
      ...
      setFlash({ type: 'success', message: copy.copyPosterSuccess })
    } catch (error) {
      setFlash({ type: 'error', message: error instanceof Error ? error.message : copy.copyPosterError })
    } finally {
      setBusy(null)
    }
  }
    ...
    <section className="mt-10 rounded-3xl border border-[var(--border-color)] bg-[var(--surface-soft)]/70 p-4 sm:p-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-3 text-sm text-[var(--text-subtle)]">
          <span className="rounded-full bg-white/80 px-3 py-1">{copy.readStatLabel} {displayReadCount}</span>
          <span className="rounded-full bg-white/80 px-3 py-1">{copy.likeStatLabel} {summary.likeCount}</span>
          <span className="rounded-full bg-white/80 px-3 py-1">{copy.shareStatLabel} {summary.shareCount}</span>
          <span className="rounded-full bg-white/80 px-3 py-1">{copy.commentStatLabel} {summary.commentCount}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {busy === 'like' ? copy.processingLabel : summary.likedByVisitor ? copy.likedActionLabel : copy.likeActionLabel}
          </button>
          <button
            type="button"
          ...
          >
            {busy === 'link' ? copy.copyingLinkLabel : copy.copyLinkActionLabel}
          </button>
          <button
            type="button"
            ...
            className="rounded-full border border-[var(--border-color)] bg-white/80 px-4 py-2 text-sm text-[var(--text-secondary)] transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]"
          >
            {busy === 'image' ? copy.posterGeneratingLabel : copy.posterActionLabel}
          </button>
          {showCommentLink ? (
            <a
              href="#comments"
      ...
      {flash ? (
        ...
        <p className="mt-3 text-xs text-[var(--text-faint)]">
          {fillTextTemplate(copy.interactionHint, { title })}
        </p>
      )}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-slate-900">{copy.sharePreviewTitle}</p>
                <p className="mt-1 text-xs text-slate-500">{copy.sharePreviewSubtitle}</p>
              </div>
              <button
                type="button"
                onClick={closeSharePreview}
            ...
            <div className="border-t border-slate-100 px-4 py-3">
              <div className="flex gap-2">
                ...
                >
                  {busy === 'image' ? copy.copyPosterCopyingLabel : copy.copyPosterActionLabel}
                </button>
                <button
                  type="button"
                  onClick={closeSharePreview}
              ...
              <p className="mt-2 text-center text-xs text-slate-500">{copy.posterMobileHint}</p>
            </div>
          </div>
        </div>
      ) : null}
