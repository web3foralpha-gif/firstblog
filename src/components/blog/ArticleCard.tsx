...
export default function ArticleCard({
  ...
  return (
    <Link href={href || `/article/${slug}`} className="block group">
      <article className="card min-h-[172px] overflow-hidden px-5 py-5 transition-transform active:scale-[0.99] sm:min-h-[186px] sm:px-6 sm:py-6">
        <div
          className={
            hasCover
              ? 'grid h-full items-center gap-5 sm:grid-cols-[minmax(0,1fr)_188px] sm:gap-6 lg:grid-cols-[minmax(0,1fr)_224px]'
          ...
          <div className="flex h-full min-w-0 flex-col justify-between">
            <div>
              <div className="mb-3 flex items-center gap-2.5 flex-wrap">
                ...
                <h2 className="min-w-0 flex-1 font-serif text-[17px] font-medium leading-8 text-[var(--text-primary)] transition-colors line-clamp-2 group-hover:text-[var(--accent)] sm:text-[18px]">
                  {title}
                </h2>
              </div>

              {previewText && (
                <p className="line-clamp-2 text-[14px] leading-7 text-[var(--text-muted)]">{previewText}</p>
              )}
            </div>

            <div className="mt-4 flex items-center flex-wrap gap-2.5 sm:gap-3">
              <time className="theme-chip !px-3.5 !py-1.5 !text-[12px] !shadow-none">{formatDate(createdAt)}</time>
              {accessType === 'PAID' && price && (
                <span className="theme-chip !px-3.5 !py-1.5 !text-[12px] !shadow-none">¥{price}</span>
              )}
            </div>
          </div>
