...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { href: '/admin/articles/new', icon: '✏️', label: '写新文章' },
      { href: '/admin/comments',  icon: '💬', label: '评论管理' },
      { href: '/admin/guestbook', icon: '📮', label: '留言审核' },
      { href: '/admin/media',     icon: '🖼️', label: '媒体库' },
    ],
  },
  {
    label: '互动数据',
    items: [
      { href: '/admin/analytics', icon: '📊', label: '访问统计' },
      { href: '/admin/sunflower', icon: '🌻', label: '向日葵' },
      { href: '/admin/payments',  icon: '💳', label: '打赏记录' },
    ],
  },
  {
    label: '系统设置',
    items: [
      ...
      { href: '/admin/backup',   icon: '💾', label: '备份导出' },
    ],
  },
]
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-4">
        {/* 仪表盘 */}
        <Link href="/admin"
          className={`nav-item ${isActive('/admin') && pathname === '/admin' ? 'nav-item-active' : ''} ${collapsed ? 'justify-center px-2' : ''}`}
